...
public class BoxDemo {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < n; ++i) {
			...
			// if this is the first instance then we add the object directly to set
			if(i == 0) {
				set.add(arr[i]);
			}
			// else we will check whether a box with the same volume exists in the set or not
			else
			{
				for(Box ob : set) {
					// break if same box is already present
